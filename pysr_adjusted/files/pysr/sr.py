...
def _has_inline_operator_definition(operators: dict[int, list[str]] | None) -> bool:
    if operators is None:
        return False
    return any("(" in op for op_list in operators.values() for op in op_list)
...
class PySRRegressor(MultiOutputMixin, RegressorMixin, BaseEstimator):
    ...
    **kwargs : dict
    ...
    def __init__(
        ...
        *,
        backend: Literal["julia", "python"] | None = None,
        ...
        **kwargs,
        ...
        _be = os.environ.get("PYSR_BACKEND")
        self.backend = backend if backend is not None else (_be if _be is not None else "python")
    ...
    def _validate_and_modify_params(self) -> _DynamicallySetParams:
        ...
        if self.expression_spec is not None:
            raise ValueError(
                "expression_spec templates require Julia and are not supported by the Python backend."
            )

        requested_operators = self.operators
        if requested_operators is None:
            requested_operators = {
                2: self.binary_operators or [],
                1: self.unary_operators or [],
            }
        if _has_inline_operator_definition(requested_operators):
            raise ValueError(
                "Inline custom operator definitions require Julia and are not supported by the Python backend. "
                "Use the built-in operator names listed in python_backend/data/sr-operator-registry.yaml."
            )
    ...
    def _set_equations_from_result(self, result: dict) -> None:
        """Populate equation_file_contents_ from the Python backend result dict."""
        import pandas as pd
        from python_backend.expr import parse_canonical, node_to_sympy

        n_outputs = result.get("n_outputs", 1)
        n_features = result.get("dataset_manifest", {}).get("n_features", 1)
        variable_names = [f"x{i}" for i in range(n_features)]

        if n_outputs > 1:
            hof_per_output: list[pd.DataFrame] = []
            best_list = result.get("best", [{}] * n_outputs)
            for j in range(n_outputs):
                entry = best_list[j] if j < len(best_list) else {}
                canonical = str(entry.get("canonical_expression", ""))
                try:
                    node = parse_canonical(canonical)
                    equation = node_to_sympy(node, variable_names=variable_names)
                    equation_str = str(equation)
                except Exception:
                    equation_str = ""
                rows = [{
                    "complexity": entry.get("complexity", 1),
                    "loss": max(float(entry.get("loss", 0.0)), 1e-16),
                    "equation": equation_str,
                }]
                hof_per_output.append(pd.DataFrame(rows))
            self.equation_file_contents_ = hof_per_output
        else:
            ...
            self.equation_file_contents_ = [pd.DataFrame(rows)]
    ...
    def _run_python_backend(
        ...
        # Map PySR params to BackendOptions
        kw: dict[str, Any] = {}
        kw["binary_operators"] = list(runtime_params.operators.get(2, []))
        kw["unary_operators"] = list(runtime_params.operators.get(1, []))
        kw["niterations"] = int(self.niterations)
        kw["population_size"] = int(self.population_size)
        kw["populations"] = int(self.populations)
        kw["maxsize"] = int(self.maxsize)
        kw["topn"] = int(self.topn)
        kw["maxdepth"] = int(self.maxdepth) if self.maxdepth is not None else 10
        kw["tournament_selection_n"] = int(self.tournament_selection_n)
        kw["ncycles_per_iteration"] = int(self.ncycles_per_iteration)
        kw["alpha"] = float(self.alpha)
        kw["parsimony"] = float(self.parsimony)
        kw["migration"] = bool(self.migration)
        kw["hof_migration"] = bool(self.hof_migration)
        kw["fraction_replaced"] = float(self.fraction_replaced)
        kw["fraction_replaced_hof"] = float(self.fraction_replaced_hof)
        kw["fraction_replaced_guesses"] = float(self.fraction_replaced_guesses)
        kw["crossover_probability"] = float(self.crossover_probability)
        kw["should_simplify"] = bool(self.should_simplify)
        kw["use_frequency"] = bool(self.use_frequency)
        kw["use_frequency_in_tournament"] = bool(self.use_frequency_in_tournament)
        kw["adaptive_parsimony_scaling"] = float(self.adaptive_parsimony_scaling)
        kw["skip_mutation_failures"] = bool(self.skip_mutation_failures)
        kw["perturbation_factor"] = float(self.perturbation_factor)
        kw["probability_negate_constant"] = float(self.probability_negate_constant)
        kw["optimize_probability"] = float(self.optimize_probability)
        kw["optimizer_iterations"] = int(self.optimizer_iterations)
        kw["optimizer_nrestarts"] = int(self.optimizer_nrestarts)
        kw["warm_start"] = bool(self.warm_start)
        kw["deterministic"] = bool(self.deterministic)
        kw["bumper"] = bool(self.bumper)
        kw["fast_cycle"] = bool(self.fast_cycle)
        kw["turbo"] = bool(self.turbo)
        kw["warmup_maxsize_by"] = int(self.warmup_maxsize_by) if self.warmup_maxsize_by is not None else 0
        kw["loss_scale"] = 1.0
        kw["annealing"] = bool(self.annealing)
        kw["elementwise_loss"] = self.elementwise_loss
        kw["early_stop_condition"] = str(self.early_stop_condition) if self.early_stop_condition else ""
        kw["constraints"] = self.constraints
        kw["nested_constraints"] = self.nested_constraints
        kw["guesses"] = self.guesses
        kw["batching"] = self.batching if isinstance(self.batching, bool) else False
        kw["batch_size"] = int(self.batch_size) if self.batch_size is not None else 0
        kw["denoise"] = bool(self.denoise)
        kw["select_k_features"] = self.select_k_features
        kw["dimensional_constraint_penalty"] = self.dimensional_constraint_penalty
        kw["dimensionless_constants_only"] = bool(self.dimensionless_constants_only)
        kw["complexity_of_constants"] = int(self.complexity_of_constants) if self.complexity_of_constants is not None else 1
        kw["complexity_of_variables"] = int(self.complexity_of_variables) if isinstance(self.complexity_of_variables, (int, float)) else 1
        kw["model_selection"] = self.model_selection
        kw["search_algorithm"] = "regularized_evolution"

        options = BackendOptions(**kw)

        # Pass saved_state for warm-start support
        _saved_state = getattr(self, "_saved_state", None)

        backend = PythonSRBackend()
        result = backend.equation_search(
            X, y, options=options, seed=seed, weights=weights,
            saved_state=_saved_state,
        )

        self._saved_state = result.get("saved_state")
        self._set_equations_from_result(result)
    ...
    def _run(
        ...
        if self.backend == "python":
            return self._run_python_backend(
                X, y, runtime_params, weights, seed,
            )
    ...
    def fit(
        ...
        *,
        ...
        if self.warm_start and (
            hasattr(self, "julia_state_stream_") or hasattr(self, "_saved_state")
        ):
            pass
        else:
            if hasattr(self, "julia_state_stream_") or hasattr(self, "_saved_state"):
                warnings.warn(
