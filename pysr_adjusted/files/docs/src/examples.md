...
> **Python backend is the default.** PySR now ships with a pure-Python
> search engine. No Julia installation is required. The legacy Julia
> backend remains available via `backend="julia"` if `juliacall` is
> installed.
...
Before the search engine is launched, you can see the string:
...
## 7. Julia packages and types (optional Julia backend)

PySR's default search engine is pure Python. If you need to interface
with Julia libraries, you can opt in to the legacy Julia backend
by installing `juliacall` and setting `backend="julia"`.
PySR's Julia backend uses [SymbolicRegression.jl](https://github.com/MilesCranmer/SymbolicRegression.jl),
a pure Julia package that can interface easily with any other
Julia package.
