...
PySR ships with a **pure-Python backend** by default (no Julia required).
The original backend was written as a Julia package under the name [SymbolicRegression.jl](https://github.com/MilesCranmer/SymbolicRegression.jl),
and is still available via `backend="julia"` if `juliacall` is installed.

> **Note:** The instructions below are for customizing the legacy Julia backend.
> Most users should use the Python backend, whose source is in `python_backend/`.
