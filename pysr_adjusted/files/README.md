...
# PySR: High-Performance Symbolic Regression in Python (Julia Optional)
...
PySR ships with a pure-Python search engine by default.
It was originally developed alongside the Julia library [SymbolicRegression.jl](https://github.com/MilesCranmer/SymbolicRegression.jl),
and can optionally use that Julia backend if `juliacall` is installed.
...
The default Python backend requires no Julia installation.
(Optionally, install `juliacall` for the legacy Julia backend.)
...
Internally, this uses a pure-Python search engine by default (no Julia needed).
To use the legacy Julia backend, pass `backend="julia"` and ensure `juliacall` is installed.
